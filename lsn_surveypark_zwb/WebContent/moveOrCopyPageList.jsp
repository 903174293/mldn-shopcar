		...
		<s:iterator var="s" value="mySurveys">
			...
			<s:iterator var="p" value="#s.pages" status="st">
				...
				<s:if test="#pId == srcPid">
					<!-- 设置字符串变量值,保持颜色 -->
					<s:set var="bgcolor" value='"rgb(0,0,0)"' />
				</s:if>
				<s:else>
				...
				<tr bgcolor='<s:property value="#bgcolor"/>'>
					...
					<td><s:if test="#pId != srcPid">
					
							<s:form name="form%{#pId}"
								action="MoveOrCopyPageAction_doMoveOrCopyPage" method="post">
								
								<s:hidden name="srcPid" />
								<s:hidden name="targPid" value="%{#pId}" />
								<!-- 当移动/复制完成后,需要重定向到目标调查的设计页面 -->
								<s:hidden name="sid" value="%{#sId}" />
								<s:radio list="#{0:'之前',1:'之后'}" listKey="key" listValue="value"
									name="pos" />
